...
the `__links` section of the JSON response.
...
[source,ruby]
----
hal_link :link_name, key: 'link_key', realize_class: 'TargetResourceClass'
----

`link_name`:: The name of the link, which will be used to access the link in the
resource object.
`link_key`:: The key of the link in the JSON response. This is the name of the
link as it appears in the `_links` section of the HAL resource.
`TargetResourceClass`:: The class of the target resource that the link points to. This
is used to resolve the link to the associated resource.

The `_links` section is modeled as a dynamically created link set class, named
after the resource's class name (with an appended `Links` string), which in turn
contains the defined links to other resources. The link set class is inherited
from `Lutaml::Model::Serializable`.

[example]
====
A HAL resource of class `Product` will have a link set of class `ProductLinks`
which contains the `self` and `category` links as its attributes.
====

Each link object of the link set is provided as a `Lutaml::Hal::Link` object.

[example]
====
For an instance of `Product`, the library will provide:
...
* calling `product.links.self` will return an instance of `Link`.
